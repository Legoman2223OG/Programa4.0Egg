...
public class AutorServicio extends DAO<Autor>{
    ...
    public void DarDeAltaAutor(int idautor,boolean alta){
        try{
            if(em.find(Autor.class, idautor) == null)
                throw new Exception("No se encontro el autor con id "+idautor);
            Autor autor = em.find(Autor.class, idautor);
            autor.setAlta(alta);
            super.ModificarEntidad(autor);
        }
        catch(Exception e){
            System.out.println("No se pudo dar de alta el autor.\nRazon: "+e.getMessage());
        }
    }

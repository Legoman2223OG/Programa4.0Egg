...
public class EditorialServicio extends DAO<Editorial>{
    ...
    public void DarDeAltaEditorial(int ideditorial,boolean alta){
        try{
            if(em.find(Editorial.class, ideditorial) == null)
                throw new Exception("No se encontro la editorial con id "+ideditorial);
            Editorial editorial = em.find(Editorial.class, ideditorial);
            editorial.setAlta(alta);
            super.ModificarEntidad(editorial);
        }
        catch(Exception e){
            System.out.println("No se pudo dar de alta la editorial.\nRazon: "+e.getMessage());
        }
    }
